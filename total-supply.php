...
$json_response = file_get_contents('https://explorer.enix.ai/api/?module=stats&action=ethsupply');
$json_data = json_decode($json_response, true);

// Pass result into 
if($json_data['message'] == 'OK'){
    echo convertCurrency($json_data['result'], 'gwei');
}

function convertCurrency(float $amount, string $from = 'wei', string $to = 'ether') {
    // relative to Ether
    $convertTabe = [
    ...
    return sprintf('%f', (($convertTabe[$to] * $amount) / $convertTabe[$from]));
}
